...
// V11  2022-6-7
//  - fixed scaling parameters; medium/large was reversed
// 2022-6-8
...
//=== full millitome with the requested parameters, for 3d printing, use laterality parameter to control
if (laterality != 2) {
    if (laterality == 0) {
        complete_bottom();          // create complete bottom half
    } else {
        complete_top();             // complete top half
    }
}
...
scaling_factor  = scaling_array[organ_scale];
...
// male organ list, dimensions are rounded up
organ_list_m = [
...
//            text(str(scaling_factor*100,"% ",organ_file),size=font_size*1.2);
    ...
    if (output_flag == 1) {
        echo (str(
            ">>",
            coa[_gender][gender],",",
            coa[_organ1][organ_id],",",
            coa[_organ2][organ_id],",",
            block_size,",",
            coa[_organ_scale][organ_scale],",",
            coa[_laterality][laterality],",",        
